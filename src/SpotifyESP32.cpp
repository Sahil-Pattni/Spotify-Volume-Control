...
#include "../include/SpotifyESP32.h"

SpotifyESP32::SpotifyESP32(Client &client)
{
    this->client = &client;
}

SpotifyESP32::SpotifyESP32(Client &client, char *bearerToken)
{
    this->client = &client;
    sprintf(this->_bearerToken, "Bearer %s", bearerToken);
}

SpotifyESP32::SpotifyESP32(Client &client, const char *clientId, const char *clientSecret, const char *refreshToken)
{
    this->client = &client;
...
int SpotifyESP32::makeRequestWithBody(const char *type, const char *command, const char *authorization, const char *body, const char *contentType, const char *host)
{
    client->flush();
#ifdef SPOTIFY_DEBUG
    Serial.println(host);
...
int SpotifyESP32::makePutRequest(const char *command, const char *authorization, const char *body, const char *contentType, const char *host)
{
    return makeRequestWithBody("PUT ", command, authorization, body, contentType);
}

int SpotifyESP32::makePostRequest(const char *command, const char *authorization, const char *body, const char *contentType, const char *host)
{
    return makeRequestWithBody("POST ", command, authorization, body, contentType, host);
}

int SpotifyESP32::makeGetRequest(const char *command, const char *authorization, const char *accept, const char *host)
{
    client->flush();
    client->setTimeout(SPOTIFY_TIMEOUT);
    if (!client->connect(host, portNumber))
...
void SpotifyESP32::setRefreshToken(const char *refreshToken)
{
...
bool SpotifyESP32::refreshAccessToken()
{
    char body[300];
...
bool SpotifyESP32::checkAndRefreshAccessToken()
{
...
const char *SpotifyESP32::requestAccessTokens(const char *code, const char *redirectUrl)
{
...
bool SpotifyESP32::play(const char *deviceId)
{
    char command[100] = SPOTIFY_PLAY_ENDPOINT;
    return playerControl(command, deviceId);
}

bool SpotifyESP32::playAdvanced(char *body, const char *deviceId)
{
    char command[100] = SPOTIFY_PLAY_ENDPOINT;
    return playerControl(command, deviceId, body);
}

bool SpotifyESP32::pause(const char *deviceId)
{
    char command[100] = SPOTIFY_PAUSE_ENDPOINT;
    return playerControl(command, deviceId);
}

bool SpotifyESP32::setVolume(int volume, const char *deviceId)
{
    char command[125];
...
bool SpotifyESP32::toggleShuffle(bool shuffle, const char *deviceId)
{
    char command[125];
...
bool SpotifyESP32::setRepeatMode(RepeatOptions repeat, const char *deviceId)
{
    char command[125];
    char repeatState[10];
    switch (repeat)
...
bool SpotifyESP32::playerControl(char *command, const char *deviceId, const char *body)
{
    if (deviceId[0] != 0)
    {
...
bool SpotifyESP32::playerNavigate(char *command, const char *deviceId)
{
    if (deviceId[0] != 0)
    {
        char deviceIdBuff[50];
...
bool SpotifyESP32::nextTrack(const char *deviceId)
{
    char command[100] = SPOTIFY_NEXT_TRACK_ENDPOINT;
    return playerNavigate(command, deviceId);
}

bool SpotifyESP32::previousTrack(const char *deviceId)
{
    char command[100] = SPOTIFY_PREVIOUS_TRACK_ENDPOINT;
    return playerNavigate(command, deviceId);
}
bool SpotifyESP32::seek(int position, const char *deviceId)
{
...
bool SpotifyESP32::transferPlayback(const char *deviceId, bool play)
{
    char body[100];
...
int SpotifyESP32::getCurrentlyPlaying(processCurrentlyPlaying currentlyPlayingCallback, const char *market)
{
...
int SpotifyESP32::getPlayerDetails(processPlayerDetails playerDetailsCallback, const char *market)
{
...
int SpotifyESP32::getDevices(processDevices devicesCallback)
{
...
int SpotifyESP32::searchForSong(String query, int limit, processSearch searchCallback, SearchResult results[])
{
...
int SpotifyESP32::commonGetImage(char *imageUrl)
{
#ifdef SPOTIFY_DEBUG
...
bool SpotifyESP32::getImage(char *imageUrl, Stream *file)
{
...
bool SpotifyESP32::getImage(char *imageUrl, uint8_t **image, int *imageLength)
{
    int totalLength = commonGetImage(imageUrl);
...
int SpotifyESP32::getContentLength()
{
...
void SpotifyESP32::skipHeaders(bool tossUnexpectedForJSON)
{
    // Skip HTTP headers
    if (!client->find("\r\n\r\n"))
    {
...
int SpotifyESP32::getHttpStatusCode()
{
    char status[32] = {0};
...
void SpotifyESP32::lateInit(const char *clientId, const char *clientSecret, const char *refreshToken)
{
    this->_clientId = clientId;
    this->_clientSecret = clientSecret;
    setRefreshToken(refreshToken);
